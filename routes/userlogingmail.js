...
router.post("/", async (req, res) => {
  ...
  const userWithemail = await userauth.findOne({ where: { email } }).catch((err) => {
    console.log("Error: ", err);
  });

  console.log("user in login backend route is ",userWithemail);
  ...
  // create JWTs
    const accessToken = jwt.sign(
      { "email": userWithemail.dataValues.email},
      process.env.ACCESS_TOKEN_SECRET,
      { expiresIn: '10s' }
  );
  const refreshToken = jwt.sign(
      { "username": userWithemail.dataValues.email },
  ...
  // Update the user record in database with the new refresh token
    ...
    if (updatedUser[0] === 1) {
      ...
      res.status(200).json({ message: 'Login successful', accessToken: accessToken, id:userWithemail.id,name:userWithemail.name,email:userWithemail.email,userID : userWithemail.userID,loginroute : userWithemail.loginroute});
    } else {
